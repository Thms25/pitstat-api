...
def scrape_drivers():
    ...
    try:
        print("Scraping drivers started")
        ...
        for driver_element in driver_elements:
            ...
            num_and_flag = driver_document.select_one('figcaption')
            num = num_and_flag.select_one('p').text
            driver_info['flag'] = num_and_flag.select_one('img')['src']
            ...
            rank += 1
            print(f"Scraped driver: {full_name} - {num}")
        print(f"Scraped {len(drivers)} drivers")
        return drivers
